...
GUID GetGuidArchiveReader(const CompressionFormatEnum& format)
{
...
void SevenZipCompressor::SetPassword(const wchar_t *password)
{
	m_password = password;
	m_passwordSet = true;
}
...
void SevenZipCompressor::CompressFilesToArchive( const TString& pathPrefix, const std::vector< FilePathInfo >& filePaths )
{
	...
	m_library.CreateObject(SevenZip::GetGuidArchiveReader(m_format), IID_IOutArchive, reinterpret_cast< void** >(&archiver));
