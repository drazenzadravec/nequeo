...
public:
	EXPORT_LIVE_MEDIA_SERVER_API static RTSPClient* createNew(UsageEnvironment& env, char const* rtspURL,
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendDescribeCommand(responseHandler* responseHandler, Authenticator* authenticator = NULL);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendOptionsCommand(responseHandler* responseHandler, Authenticator* authenticator = NULL);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendAnnounceCommand(char const* sdpDescription, responseHandler* responseHandler, Authenticator* authenticator = NULL);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendSetupCommand(MediaSubsession& subsession, responseHandler* responseHandler,
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendPlayCommand(MediaSession& session, responseHandler* responseHandler,
      ...
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendPlayCommand(MediaSubsession& subsession, responseHandler* responseHandler,
			   double start = 0.0f, double end = -1.0f, float scale = 1.0f,
			   Authenticator* authenticator = NULL);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendPauseCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues an aggregate RTSP "PAUSE" command on "session", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendPauseCommand(MediaSubsession& subsession, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues a RTSP "PAUSE" command on "subsession", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)

  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendRecordCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues an aggregate RTSP "RECORD" command on "session", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendRecordCommand(MediaSubsession& subsession, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues a RTSP "RECORD" command on "subsession", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)

  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendTeardownCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues an aggregate RTSP "TEARDOWN" command on "session", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendTeardownCommand(MediaSubsession& subsession, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues a RTSP "TEARDOWN" command on "subsession", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)

  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendSetParameterCommand(MediaSession& session, responseHandler* responseHandler,
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sendGetParameterCommand(MediaSession& session, responseHandler* responseHandler, char const* parameterName,
				   Authenticator* authenticator = NULL);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API void sendDummyUDPPackets(MediaSession& session, unsigned numDummyPackets = 2);
  EXPORT_LIVE_MEDIA_SERVER_API void sendDummyUDPPackets(MediaSubsession& subsession, unsigned numDummyPackets = 2);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API void setSpeed(MediaSession& session, float speed = 1.0f);
      // Set (recorded) media download speed to given value to support faster download using 'Speed:'
      // option on 'PLAY' command.

  EXPORT_LIVE_MEDIA_SERVER_API Boolean changeResponseHandler(unsigned cseq, responseHandler* newResponseHandler);
  ...
  EXPORT_LIVE_MEDIA_SERVER_API int socketNum() const { return fInputSocketNum; }

  EXPORT_LIVE_MEDIA_SERVER_API static Boolean lookupByName(UsageEnvironment& env,
			      char const* sourceName,
			      RTSPClient*& resultClient);

  EXPORT_LIVE_MEDIA_SERVER_API static Boolean parseRTSPURL(UsageEnvironment& env, char const* url,
  ...
  EXPORT_LIVE_MEDIA_SERVER_API void disallowBasicAuthentication() { fAllowBasicAuthentication = False; }
  ...
  EXPORT_LIVE_MEDIA_SERVER_API unsigned sessionTimeoutParameter() const { return fSessionTimeoutParameter; }

  EXPORT_LIVE_MEDIA_SERVER_API char const* url() const { return fBaseURL; }
