...
public:
	EXPORT_LIVE_MEDIA_SERVER_API static SIPClient* createNew(UsageEnvironment& env,
		unsigned char desiredAudioRTPPayloadFormat,
		char const* mimeSubtype = NULL,
		int verbosityLevel = 0,
		char const* applicationName = NULL);

	EXPORT_LIVE_MEDIA_SERVER_API void setProxyServer(unsigned proxyServerAddress,
		portNumBits proxyServerPortNum);

	EXPORT_LIVE_MEDIA_SERVER_API void setClientStartPortNum(portNumBits clientStartPortNum) {
		fClientStartPortNum = clientStartPortNum;
	}

	EXPORT_LIVE_MEDIA_SERVER_API char* invite(char const* url, Authenticator* authenticator = NULL);
	// Issues a SIP "INVITE" command
	// Returns the session SDP description if this command succeeds
	EXPORT_LIVE_MEDIA_SERVER_API char* inviteWithPassword(char const* url,
	...
	EXPORT_LIVE_MEDIA_SERVER_API Boolean sendACK(); // on current call
	Boolean sendBYE(); // on current call

	EXPORT_LIVE_MEDIA_SERVER_API static Boolean parseSIPURL(UsageEnvironment& env, char const* url,
		NetAddress& address, portNumBits& portNum);
	// (ignores any "<username>[:<password>]@" in "url")
	EXPORT_LIVE_MEDIA_SERVER_API static Boolean parseSIPURLUsernamePassword(char const* url,
		char*& username,
		char*& password);
	EXPORT_LIVE_MEDIA_SERVER_API char const* getInviteSdpReply() const { return fInviteSDPDescriptionReturned; }
