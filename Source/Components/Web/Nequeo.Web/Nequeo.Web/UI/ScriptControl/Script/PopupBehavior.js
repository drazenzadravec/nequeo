...
Nequeo.Web.UI.ScriptControl.PopupBehavior = function(element) {
    Nequeo.Web.UI.ScriptControl.PopupBehavior.initializeBase(this, [element]);
    this._x = 0;
    this._y = 0;
    this._positioningMode = Nequeo.Web.UI.ScriptControl.PositioningMode.Absolute;
...
Nequeo.Web.UI.ScriptControl.PopupBehavior.prototype = {
    initialize: function() {
        Nequeo.Web.UI.ScriptControl.PopupBehavior.callBaseMethod(this, 'initialize'); this._hidePopup(); this.get_element().style.position = "absolute"; this._onShowEndedHandler = Function.createDelegate(this, this._onShowEnded); this._onHideEndedHandler = Function.createDelegate(this, this._onHideEnded);
    },
    dispose: function() {
        ...
        this._onHideEndedHandler = null; this._onHide = null; Nequeo.Web.UI.ScriptControl.PopupBehavior.callBaseMethod(this, 'dispose');
    },
    show: function() {
        if (this._visible) {
            return;
        ...
        this._visible = true; var element = this.get_element(); $common.setVisible(element, true); this.setupPopup(); if (this._onShow) {
            $common.setVisible(element, false); this.onShow();
        } else {
    ...
    getBounds: function() {
        var element = this.get_element(); var offsetParent = element.offsetParent || document.documentElement; var diff; var parentBounds; if (this._parentElement) {
            parentBounds = $common.getBounds(this._parentElement); var offsetParentLocation = $common.getLocation(offsetParent); diff = { x: parentBounds.x - offsetParentLocation.x, y: parentBounds.y - offsetParentLocation.y };
        } else {
        ...
        var position; switch (this._positioningMode) {
            case Nequeo.Web.UI.ScriptControl.PositioningMode.Center:
                position = {
                    x: Math.round(parentBounds.width / 2 - width / 2),
                    y: Math.round(parentBounds.height / 2 - height / 2)
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.BottomLeft:
                position = {
                    x: 0,
                    y: parentBounds.height
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.BottomRight:
                position = {
                    x: parentBounds.width - width,
                    y: parentBounds.height
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.TopLeft:
                position = {
                    x: 0,
                    y: -element.offsetHeight
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.TopRight:
                position = {
                    x: parentBounds.width - width,
                    y: -element.offsetHeight
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.Right:
                position = {
                    x: parentBounds.width,
                    y: 0
                }; break; case Nequeo.Web.UI.ScriptControl.PositioningMode.Left:
                position = {
    ...
    set_onShow: function(value) {
        if (!this._onShow) {
            this._onShow = new Nequeo.Web.UI.ScriptControl.Animation.GenericAnimationBehavior(this.get_element()); this._onShow.initialize();
        }
    ...
    set_onHide: function(value) {
        if (!this._onHide) {
            this._onHide = new Nequeo.Web.UI.ScriptControl.Animation.GenericAnimationBehavior(this.get_element()); this._onHide.initialize();
        }
    ...
    get_parentElement: function() {
        if (!this._parentElement && this._parentElementID) {
            this.set_parentElement($get(this._parentElementID)); Sys.Debug.assert(this._parentElement != null, String.format(Nequeo.Web.UI.ScriptControl.Resources.PopupExtender_NoParentElement, this._parentElementID));
        }
...
Nequeo.Web.UI.ScriptControl.PopupBehavior.registerClass('Nequeo.Web.UI.ScriptControl.PopupBehavior', Nequeo.Web.UI.ScriptControl.BehaviorBase); Nequeo.Web.UI.ScriptControl.PositioningMode = function() {
    throw Error.invalidOperation();
}
Nequeo.Web.UI.ScriptControl.PositioningMode.prototype = {
    Absolute: 0,
    Center: 1,
    BottomLeft: 2,
    BottomRight: 3,
...
Nequeo.Web.UI.ScriptControl.PositioningMode.registerEnum('Nequeo.Web.UI.ScriptControl.PositioningMode');
