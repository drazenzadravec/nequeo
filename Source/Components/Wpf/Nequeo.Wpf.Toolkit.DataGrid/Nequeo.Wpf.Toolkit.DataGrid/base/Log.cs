...
namespace Nequeo.Wpf.DataGrid
{
  internal class Log
  {
    ...
    internal void StartUp( string gridUniqueName = "" )
    {
      ...
      {
        m_defaultPath = Path.Combine( Environment.GetFolderPath( Environment.SpecialFolder.ApplicationData ), @"NequeoLog" );
      }
    ...
    private void WriteException( Exception e )
    {
      if( e != null )
      {
        ...
        if( stackTraceString.IndexOf( "Nequeo", StringComparison.InvariantCultureIgnoreCase ) >= 0 )
        {
          lock( this.SyncRoot )
          {
