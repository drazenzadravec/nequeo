...
namespace Nequeo.Wpf.DataGrid
{
  ...
  public struct ColumnWidth
  {
    ...
    public ColumnWidth( double value, ColumnWidthUnitType type )
    {
      ...
      if( ( Nequeo.Wpf.Math.DoubleUtil.AreClose( value, 0d ) ) && ( type == ColumnWidthUnitType.Star ) )
    ...
    public static bool operator ==( ColumnWidth columnWidth1, ColumnWidth columnWidth2 )
    {
      return 
        ( ( columnWidth1.UnitType == columnWidth2.UnitType ) && 
          ( Nequeo.Wpf.Math.DoubleUtil.AreClose( columnWidth1.Value, columnWidth2.Value ) ) );
    }

    public static bool operator !=( ColumnWidth columnWidth1, ColumnWidth columnWidth2 )
    {
      if( columnWidth1.UnitType == columnWidth2.UnitType )
        return !Nequeo.Wpf.Math.DoubleUtil.AreClose( columnWidth1.Value, columnWidth2.Value );
