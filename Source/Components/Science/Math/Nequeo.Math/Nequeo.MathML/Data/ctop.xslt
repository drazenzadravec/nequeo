...
-->
  ...
  <xsl:template mode="c2p" match="mml:cn[@type='complex-cartesian']">
    <mml:mrow>
      <mml:mn>
        <xsl:apply-templates mode="c2p" select="text()[1]"/>
      </mml:mn>
      <mml:mo>+</mml:mo>
      <mml:mn>
        <xsl:apply-templates mode="c2p" select="text()[2]"/>
      </mml:mn>
      <mml:mo>
        <!--&#8290;-->
        <!--invisible times-->
  ...
  <xsl:template mode="c2p" match="mml:cn[@type='e-notation']">
    <mml:mn>
      <xsl:apply-templates mode="c2p" select="text()[1]"/>E<xsl:apply-templates mode="c2p" select="text()[2]"/>
    </mml:mn>
  </xsl:template>
  ...
  <xsl:template mode="c2p" match="mml:apply[*[1][self::mml:root]]">
    <mml:mroot>
      ...
      <mml:mrow>
        <xsl:apply-templates mode="c2p" select="mml:degree/*"/>
      </mml:mrow>
    </mml:mroot>
  </xsl:template>
  ...
  <xsl:template mode="c2p" match="mml:apply[*[1][self::mml:forall]]">
    <mml:mrow>
      ...
      <mml:mrow>
        <xsl:apply-templates mode="c2p" select="mml:bvar[not(current()/mml:condition)]/*|mml:condition/*"/>
      </mml:mrow>
      <mml:mo>.</mml:mo>
      <mml:mfenced>
        <xsl:apply-templates mode="c2p" select="*[last()]"/>
  ...
  <xsl:template mode="c2p" match="mml:apply[*[1][self::mml:int]]">
    <mml:mrow>
      <mml:msubsup>
        ...
        <mml:mrow>
          <xsl:apply-templates mode="c2p" select="mml:lowlimit/*|mml:interval/*[1]|mml:condition/*"/>
        </mml:mrow>
        <mml:mrow>
          <xsl:apply-templates mode="c2p" select="mml:uplimit/*|mml:interval/*[2]"/>
        </mml:mrow>
      </mml:msubsup>
      <xsl:apply-templates mode="c2p" select="*[last()]"/>
      <mml:mo>d</mml:mo>
      ...
      <xsl:choose>
        <xsl:when test="mml:bvar/mml:degree">
          ...
          <mml:mrow>
            ...
            <mml:msup>
              <xsl:apply-templates mode="c2p"
 select="mml:bvar/node()"/>
              <xsl:apply-templates mode="c2p"
 select="mml:bvar/mml:degree/node()"/>
            </mml:msup>
          </mml:mrow>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template mode="c2p" match="mml:bvar">
    ...
    <xsl:if test="following-sibling::mml:bvar">
      <mml:mo>,</mml:mo>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <mml:msub>
            <mml:mi>log</mml:mi>
            <mml:mrow>
              <xsl:apply-templates mode="c2p" select="mml:logbase/node()"/>
            </mml:mrow>
          </mml:msub>
        </xsl:otherwise>
      </xsl:choose>
    ...
    <mml:mrow>
      ...
      <xsl:choose>
        <xsl:when test="mml:condition">
          <mml:mrow>
            <xsl:apply-templates mode="c2p" select="mml:bvar/*[not(self::bvar or self::condition)]"/>
          </mml:mrow>
          <mml:mo>|</mml:mo>
          <mml:mrow>
            <xsl:apply-templates mode="c2p" select="mml:condition/node()"/>
          </mml:mrow>
        </xsl:when>
        <xsl:otherwise>
          <xsl:for-each select="*">
