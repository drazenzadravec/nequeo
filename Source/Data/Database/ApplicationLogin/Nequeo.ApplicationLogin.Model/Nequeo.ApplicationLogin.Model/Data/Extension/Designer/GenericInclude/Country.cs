// Warning 169 (Disables the 'Never used' warning)
#pragma warning disable 169
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:4.0.30319.1
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Nequeo.DataAccess.ApplicationLogin.Data.Extension
{
    using System;
    using System.Text;
    using System.Data;
    using System.Threading;
    using System.Diagnostics;
    using System.Data.SqlClient;
    using System.Data.OleDb;
    using System.Data.Odbc;
    using System.Collections;
    using System.Reflection;
    using System.Collections.Generic;
    using System.Xml.Serialization;
    using System.Runtime.Serialization;
    using System.ComponentModel;
    using System.Linq;
    using System.Linq.Expressions;
    using System.Data.Linq;
    using System.Data.Common;
    using System.Data.Linq.SqlClient;
    using System.Data.Linq.Mapping;
    using Nequeo.Data.DataType;
    using Nequeo.Data;
    using Nequeo.Data.Control;
    using Nequeo.Data.Custom;
    using Nequeo.Data.LinqToSql;
    using Nequeo.Data.DataSet;
    using Nequeo.Data.Edm;
    using Data = Nequeo.DataAccess.ApplicationLogin.Data;
    
    
    #region Country DataSet Extension Type
    /// <summary>
    /// The Country schema object class.
    /// </summary>
    public partial class Country
    {
		private Nequeo.Threading.AsyncExecutionHandler<Country> _asyncAccount = null;
        private Exception _exception = null;
		private CountryThread _threadCountryContext = null;

        private Dictionary<object, object> _callback = new Dictionary<object, object>();
        private Dictionary<object, object> _state = new Dictionary<object, object>();

		/// <summary>
        /// Gets the current async exception; else null;
        /// </summary>
        public Exception Exception
        {
            get { return _exception; }
        }

		/// <summary>
        /// Gets the Country threading context.
        /// </summary>
        public CountryThread CountryThreadContext
        {
            get { return _threadCountryContext; }
        }

		/// <summary>
        /// On create instance of Country
        /// </summary>
		partial void OnCreated()
		{
			// Start the async control.
            _asyncAccount = new Nequeo.Threading.AsyncExecutionHandler<Country>();
			_asyncAccount.AsyncError += new Threading.EventHandler<Exception>(_asyncAccount_AsyncError);
            _asyncAccount.AsyncComplete += new Threading.EventHandler<object, string>(_asyncAccount_AsyncComplete);
            _asyncAccount.InitiliseAsyncInstance(this);

			_threadCountryContext = new CountryThread(this);
			_threadCountryContext.AsyncError += new Threading.EventHandler<Exception>(_asyncAccount_AsyncError);
		}

		/// <summary>
        /// Async error
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e1"></param>
        private void _asyncAccount_AsyncError(object sender, Exception e1)
        {
            _exception = e1;
        }

		/// <summary>
        /// Function routine handler
        /// </summary>
        /// <returns>The function handler instance.</returns>
        public virtual FunctionTransaction FunctionHandler()
        {
            return new FunctionTransaction(DefaultDatabaseConnection(), _connectionType, _connectionDataType, new Nequeo.Data.SqlServer.DataAccess());
        }

		/// <summary>
        /// Delegate transaction handler.
        /// </summary>
        /// <returns>The delegate handler instance.</returns>
        public virtual DataTransactions<Data.Country> DelegateTransaction()
        {
           return new DataTransactions<Data.Country>(
                    this.Insert, 
                    this.Delete, 
                    this.Update, 
                    this.Select);
        }

		/// <summary>
        /// Country threading handler.
        /// </summary>
        public class CountryThread : Nequeo.Threading.AsyncExecutionHandler<Country>
        {
            /// <summary>
            /// Country threading handler.
            /// </summary>
            /// <param name="service">The Country type.</param>
            public CountryThread(Country service)
                : base(service) { }
        }
    }
    #endregion
}

#pragma warning restore 169
