...
namespace Nequeo.DataAccess.CloudInteraction.Data.Extension
{
    ...
    internal class UserInsertOverride : InsertDataGenericBase<Data.User>
    {
        ...
        private void EncodePassword(Data.User user, bool encode = true)
        {
            ...
            {
                // Encode password.
                user.Password = encoder.Encode(user.Password, encoder.PasswordFormat);
                user.PasswordAnswer = encoder.Encode(user.PasswordAnswer, encoder.PasswordFormat);
            }
            else
            {
                // Decode password.
                user.Password = encoder.Decode(user.Password, encoder.PasswordFormat);
                user.PasswordAnswer = encoder.Decode(user.PasswordAnswer, encoder.PasswordFormat);
            }
        }
