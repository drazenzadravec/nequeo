...
{
    /// <summary>
    /// Data extension data context code generator.
    /// </summary>
    public class CodeDomDataExtensionContext
    {
        ...
        private DataContextExtensionContainer _data = null;
        ...
        public CodeCompileUnit GenerateCode(DataContextExtensionContainer data)
        {
        ...
        /// <returns></returns>
        private DataContextExtensionContainer ConvertToUpperCase(DataContextExtensionContainer data)
        {
            DataContextExtensionContainer items = data;
            ...
            foreach (var table in _tables)
            {
                if (_data.TableList.Contains(table.TableName.ToUpper(), new ToUpperComparer()) == !_tableListExclusion)
                {
                    ...
                    tableProperty.Type = new CodeTypeReference(
                        (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + "Extension." + table.TableName);
            ...
            if (GetDatabaseViews())
            {
                foreach (var view in _views)
                {
                    if (_data.TableList.Contains(view.TableName.ToUpper(), new ToUpperComparer()) == !_tableListExclusion)
                    {
                        ...
                        // Add the condition statement.
                        tableProperty.Type = new CodeTypeReference(
                            (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + "Extension." + view.TableName);
                        tableProperty.GetStatements.Add(
                            new CodeExpressionStatement(
                                new CodeSnippetExpression("return new " +
                                    (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + "Extension." + view.TableName + "()")));
        ...
        private void AddPropertiesExtended()
        {
            ...
            // Add the condition statement.
            tableProperty.Type = new CodeTypeReference(
                (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + "Extension." + _contextName);
            tableProperty.GetStatements.Add(
                new CodeExpressionStatement(
                    new CodeSnippetExpression("return new " +
                        (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + "Extension." + _contextName + "()")));
