...
namespace Nequeo.CustomTool.CodeGenerator.Generation
{
    ...
    /// </summary>
    public class CodeDomProcedureObject
    {
        ...
        private ProcedureExtensionContainer _data = null;
        private DataObjectContainer _dataObject = null;

        IEnumerable<ProceduresResult> _procedures = null;
        IEnumerable<ProcedureColumnsResult> _columns = null;
        ...
        public CodeCompileUnit GenerateCode(ProcedureExtensionContainer data)
        {
            ...
            _procedureListExclusion = data.ProcedureListExclusion;
        ...
        /// <returns></returns>
        private ProcedureExtensionContainer ConvertToUpperCase(ProcedureExtensionContainer data)
        {
            ProcedureExtensionContainer items = data;
            for (int i = 0; i < items.ProcedureList.Count(); i++)
                items.ProcedureList[i] = items.ProcedureList[i].ToUpper();

            return items;
        }

        /// <summary>
        /// Gets the procedures from the database.
        /// </summary>
        private bool GetDatabaseProcedures()
        {
            ...
            List<ProceduresResult> list = access.GetProcedures(_databaseConnection, null,
                Common.ConnectionProvider.GetConnectionType(_connectionType), null, _dataBaseOwner);

            if (list != null && list.Count > 0)
            {
                _procedures = list.Distinct(new UniqueProcedureNameComparer());
                return true;
            }
            else
                return false;
        ...
        /// <summary>
        /// Gets the procedure columns from the database.
        /// </summary>
        /// <param name="procedureName">The procedure name.</param>
        /// <param name="owner">The schema (owner) of the object.</param>
        private bool GetDatabaseProcedureColumns(string procedureName, string owner)
        {
            // Get the table results from the database
            // and apply the distinct method on the table
            // data only return unique column names results.
            DatabaseAccess access = new DatabaseAccess();
            List<ProcedureColumnsResult> list = access.GetDatabaseProcedureColumns(
                _databaseConnection, null, procedureName, owner,
                Common.ConnectionProvider.GetConnectionType(_connectionType), null, _dataBaseOwner);

            if (list != null && list.Count > 0)
            {
                _columns = list.Distinct(new UniqueProcedureColumnNameComparer());
                return true;
            }
            else
                return false;
        ...
        private void AddMethodsFunctionHandler()
        {
            ...
            {
                if (_data.ProcedureList.Contains(procedure.ProcedureName.ToUpper(), new ToUpperComparer()) == !_procedureListExclusion)
                {
                    ...
                    createProcedureMethod.Name = ((procedure.FunctionRealName != null) ? (procedure.FunctionRealName) : (procedure.ProcedureName));
                    ...
                    createProcedureMethod.Comments.Add(new CodeCommentStatement("Execute the " + procedure.ProcedureName.ToLower() + " routine.", true));
                    createProcedureMethod.Comments.Add(new CodeCommentStatement("</summary>", true));

                    // Get the function columns
                    bool ret = GetDatabaseProcedureColumns(procedure.ProcedureName, procedure.ProcedureOwner);
                    ...
                    _procedureOwner = procedure.ProcedureOwner;
                        ...
                        foreach (var column in _columns)
                        {
                            if (!column.IsOutParameter)
                            {
                                ...
                                if (column.ColumnNullable && Common.LinqToDataTypes.GetLinqNullableType(column.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType)))
                                {
                                    ...
                                    createProcedureMethod.Parameters[createProcedureMethod.Parameters.Count - 1].CustomAttributes.Add(new CodeAttributeDeclaration("FunctionParameterAttribute",
                                        new CodeAttributeArgument(new CodeSnippetExpression("\"" + column.ColumnName + "\", \"" + column.ColumnType + "\", " +
                                            ((column.Precision != null && column.Precision > 0) ? 
                                                (((column.Length <= column.Precision)) ? column.Length : column.Precision) :
                                                    ((Common.LinqToDataTypes.GetLinqNullableType(column.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType))) ? 
                                    ...
                                    createProcedureMethod.Parameters[createProcedureMethod.Parameters.Count - 1].CustomAttributes.Add(
                                        new CodeAttributeDeclaration("FunctionParameterAttribute",
                                            new CodeAttributeArgument(
                                                new CodeSnippetExpression("\"" + column.ColumnName + "\", \"" + column.ColumnType + "\", " +
                                                    ((column.Precision != null && column.Precision > 0) ? 
                                                        (((column.Length <= column.Precision)) ? column.Length : column.Precision) :
                                                            ((Common.LinqToDataTypes.GetLinqNullableType(column.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType))) ? (column.Length) : 
                    ...
                    string functionRealNameResult = ((procedure.FunctionRealName != null) ? (procedure.FunctionRealName) : (procedure.ProcedureName));
                    System.Data.DataSet dataSet = null;
                    int hasColumnsCount = 0;

                    if (GetTableSchema(functionRealNameResult + "Result", ret, out dataSet))
                    {
                        foreach (System.Data.DataTable table in dataSet.Tables)
                            if (table.Columns.Count > 0)
                                hasColumnsCount++;

                        if (hasColumnsCount > 1)
                        {
                            int tableCount = 1;
                            foreach (System.Data.DataTable table in dataSet.Tables)
                            {
                                if (table.Columns.Count > 0)
                                {
                                    // Create each multip return type attribute.
                                    createProcedureMethod.CustomAttributes.Add(new CodeAttributeDeclaration("FunctionMultiReturnType",
                                        new CodeAttributeArgument(new CodeSnippetExpression(
                                            "typeof(" + (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName : "Data") +
                                                ".Extended." + functionRealNameResult + tableCount.ToString() + "Result), " + tableCount.ToString()))));
                                }
                                tableCount++;
                            }
                            
                            // Create the multiple return type.
                            createProcedureMethod.ReturnType = new CodeTypeReference("IFunctionMultipleResults");
                            // Assign each entity property
                            // to from the parameter.
                            createProcedureMethod.Statements.Add(new CodeMethodReturnStatement(
                                new CodeArgumentReferenceExpression("((IFunctionMultipleResults)(result.ReturnValue))")));
                        }
                        else
                        {
                    ...
                    // Add the function riutine attribute
                    createProcedureMethod.CustomAttributes.Add(new CodeAttributeDeclaration("FunctionRoutineAttribute",
                        new CodeAttributeArgument(new CodeSnippetExpression("\"" +
                            (!String.IsNullOrEmpty(procedure.ProcedureOwner) ?
                                procedure.ProcedureOwner + (!String.IsNullOrEmpty(procedure.PackageName) ? "." + procedure.PackageName : "") + "." +
                                procedure.ProcedureName : procedure.ProcedureName) +
                                "\", FunctionRoutineType.StoredProcedure" + (hasColumnsCount > 1 ? ", IsMultiResultSet = true" : "")))));
            ...
            foreach (var procedure in _procedures)
            {
                if (_data.ProcedureList.Contains(procedure.ProcedureName.ToUpper(), new ToUpperComparer()) == !_procedureListExclusion)
                {
                    ...
                    createProcedureMethod.CustomAttributes.Add(new CodeAttributeDeclaration("FunctionAttribute",
                        new CodeAttributeArgument(new CodeSnippetExpression("Name = \"" + procedure.ProcedureOwner + 
                            (procedure.PackageName != null ? "." + procedure.PackageName : "") + "." + procedure.ProcedureName + "\", IsComposable = false"))));
                    ...
                    switch(Common.ConnectionProvider.GetConnectionType(_connectionType))
                    {
                        case ConnectionProvider.ConnectionType.SqlConnection:
                            query = "\r\n\t\t\tSystem.Data.Common.DbCommand ret = Common.ExecuteQuery(" +
                                "\r\n\t\t\t\tref dataTable, \"[" + procedure.ProcedureOwner + "].[" + procedure.ProcedureName + "]\", CommandType.StoredProcedure, true, " +
                        ...
                        case ConnectionProvider.ConnectionType.OleDbConnection:
                            query = "\r\n\t\t\tSystem.Data.Common.DbCommand ret = Common.ExecuteQuery(" +
                                "\r\n\t\t\t\tref dataTable, \"" + (!String.IsNullOrEmpty(procedure.ProcedureOwner) ? (procedure.ProcedureOwner + "." + 
                                procedure.ProcedureName) : procedure.ProcedureName) + "\", CommandType.StoredProcedure, true, " +
                        ...
                        case ConnectionProvider.ConnectionType.OracleClientConnection:
                            query = "\r\n\t\t\tSystem.Data.Common.DbCommand ret = Common.ExecuteQuery(" +
                                "\r\n\t\t\t\tref dataTable, \"\"" + procedure.ProcedureOwner + (_packageName == "" ? "" : "\".\"" + _packageName) + "\".\"" +
                                procedure.ProcedureName + "\"\", CommandType.StoredProcedure, true, " +
        ...
        /// <returns></returns>
        private bool GetTableSchema(string name, bool containsPrameters, out System.Data.DataSet dataSet)
        {
            DatabaseAccess access = new DatabaseAccess();
            System.Data.DataSet dataSetData = null;
            ...
            switch (Common.ConnectionProvider.GetConnectionType(_connectionType))
            {
                case ConnectionProvider.ConnectionType.SqlConnection:
                    ...
                    try
                    {
                        access.ExecuteQuerySchema(ref dataSetData, "[" + _procedureOwner + "].[" + _procedureName + "]",
                            System.Data.CommandType.StoredProcedure, _databaseConnection, parameters.ToArray());
                    }
                    catch { }
                    ...
                    {
                        int hasColumnsCount = 0;
                        foreach (System.Data.DataTable table in dataSetData.Tables)
                            if (table.Columns.Count > 0)
                                hasColumnsCount++;
                        ...
                        if (hasColumnsCount > 1)
                        {
                            ...
                            {
                                if (table.Columns.Count > 0)
                                    CreateResultDataObject(table, name.Replace("Result", tableCount.ToString() + "Result"));
                            ...
                            return CreateResultDataObject(dataSetData.Tables[0], name);
                    }
                    else
                        return false;
                ...
                case ConnectionProvider.ConnectionType.OleDbConnection:
                    ...
                    try
                    {
                        access.ExecuteOleDbQuerySchema(ref dataSetData, "" + (!String.IsNullOrEmpty(_procedureOwner) ? _procedureOwner + "." + _procedureName + "" : _procedureName),
                ...
                case ConnectionProvider.ConnectionType.OracleClientConnection:
                                    ...
                                    ((column.Precision != null && column.Precision > 0) ? (((column.Length <= column.Precision)) ?
                                        Convert.ToInt32(column.Length) : Convert.ToInt32(column.Precision)) :
                                            ((Common.LinqToDataTypes.GetLinqNullableType(column.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType))) ? Convert.ToInt32(column.Length) :
                                                Convert.ToInt32(LinqToDataTypes.DefaultLengthValue(ConnectionProvider.GetConnectionType(_connectionType))))),
                                                System.Data.ParameterDirection.Input, true, ((Byte)(0)), ((Byte)(0)), "",
                                                System.Data.DataRowVersion.Current, Common.LinqToDataTypes.GetSqlStringValue(
                                                Common.LinqToDataTypes.GetLinqDataType(column.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType)).FullName)));
                            }
                        }
                    }
                    ...
                    try
                    {
                        access.ExecuteOracleClientQuerySchema(ref dataSetData, "\"" + _procedureOwner +
                            (_packageName == "" ? "" : "\".\"" + _packageName) + "\".\"" + _procedureName + "\"",
                            System.Data.CommandType.StoredProcedure, _databaseConnection, oParametersClient.ToArray());
                    }
                    catch { }
                ...
                case ConnectionProvider.ConnectionType.MySqlConnection:
                    List<MySql.Data.MySqlClient.MySqlParameter> mParametersClient = new List<MySql.Data.MySqlClient.MySqlParameter>();
                    ...
                    if (containsPrameters)
                    {
                        ...
                        foreach (var column in _columns)
                        {
                            if (column.IsOutParameter)
                            {
                                mParametersClient.Add(new MySql.Data.MySqlClient.MySqlParameter(column.ColumnName,
                    ...
                    try
                    {
                        access.ExecuteMySqlClientQuerySchema(ref dataSetData, "\"" + _procedureOwner +
                            (_packageName == "" ? "" : "\".\"" + _packageName) + "\".\"" + _procedureName + "\"",
                            System.Data.CommandType.StoredProcedure, _databaseConnection, mParametersClient.ToArray());
                    }
                    catch { }
            ...
            for (int i = 0; i < _dataObject.PropertyName.Count(); i++)
            {
                ...
                if ((Common.LinqToDataTypes.GetSystemType(_dataObject.PropertyType[i]).IsArray) ||
                    ...
                    valueProperty.CustomAttributes.Add(new CodeAttributeDeclaration("XmlArrayAttribute",
                        new CodeAttributeArgument(new CodeSnippetExpression("ElementName = \"" + _dataObject.PropertyName[i] + 
        ...
        private void AddMethodsMore(string name)
        {
            ...
            createObjectMethodData.Comments.Add(new CodeCommentStatement("<returns>The " + 
                (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName : "Data") + 
                ".Extended." + name + " entity.</returns>", true));
