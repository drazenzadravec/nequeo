...
namespace Nequeo.CustomTool.CodeGenerator.Generation
{
    ...
    public class CodeDomDatabaseObject
    {
        ...
        /// Gets the tables from the database.
        /// </summary>
        public IEnumerable<TablesResult> GetDatabaseTables(string databaseConnection, int connectionType, string dataBaseOwner)
        {
            // Get the table results from the database
            // and apply the distinct method on the table
            // data only return unique column names results.
            DatabaseAccess access = new DatabaseAccess();
            List<TablesResult> list = access.GetTables(databaseConnection, null,
                Common.ConnectionProvider.GetConnectionType(connectionType), null, dataBaseOwner);
        ...
        /// Gets the views from the database.
        /// </summary>
        public IEnumerable<TablesResult> GetDatabaseViews(string databaseConnection, int connectionType, string dataBaseOwner)
        {
            // Get the table results from the database
            // and apply the distinct method on the table
            // data only return unique column names results.
            DatabaseAccess access = new DatabaseAccess();
            List<TablesResult> list = access.GetViews(databaseConnection, null,
                Common.ConnectionProvider.GetConnectionType(connectionType), null, dataBaseOwner);
            ...
            if (GetDatabaseViews())
            {
                foreach (var view in _views)
                {
                    if (_data.TableList.Contains(view.TableName.ToUpper(), new ToUpperComparer()) == !_tableListExclusion)
                    {
                        ...
                        // Create a custom region.
                        CodeRegionDirective startRegion = new CodeRegionDirective(CodeRegionMode.Start, view.TableName + " Data Entity Type");
                        _targetClass.StartDirectives.Add(startRegion);

                        // Create the attributes on the class.
                        _targetClass.CustomAttributes.Add(new CodeAttributeDeclaration("DataContractAttribute",
                            new CodeAttributeArgument(new CodeSnippetExpression("Name = \"" + view.TableName + "\", IsReference=true"))));
                        _targetClass.CustomAttributes.Add(new CodeAttributeDeclaration("SerializableAttribute"));
                        _targetClass.CustomAttributes.Add(new CodeAttributeDeclaration("DataTableAttribute",
        ...
        private void AddProperties()
        {
            ...
            foreach (var result in _columns)
            {
                ...
                //    new CodeAttributeArgument(new CodeSnippetExpression("\"Gets sets, the " + result.ColumnName.ToLower() + " property for the object.\""))));
                ...
                valueProperty.CustomAttributes.Add(new CodeAttributeDeclaration("SoapElementAttribute",
                    new CodeAttributeArgument(new CodeSnippetExpression("IsNullable = " + result.ColumnNullable.ToString().ToLower()))));
            ...
            foreach (var result in _tableFKs)
            {
                if (_data.TableList.Contains(result.ForeignKeyTable.ToUpper(), new ToUpperComparer()) == !_tableListExclusion)
                {
                    ...
                    valueField.Type = new CodeTypeReference(
                        (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + result.ForeignKeyTable);
                    ...
                    CodeConditionStatement conditionalStatement = new CodeConditionStatement(
                        ...
                        new CodeExpressionStatement(
                            new CodeSnippetExpression("this." + fieldName + " = value"))});
        ...
        private void AddPropertiesRef()
        {
            ...
            foreach (var result in _tableRefs)
            {
                if (_data.TableList.Contains(result.TableName.ToUpper(), new ToUpperComparer()) == !_tableListExclusion)
                {
                    ...
                    valueField.Type = new CodeTypeReference(
                        (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + result.TableName + "[]");
                    ...
                    //// Add the attributes to the property.
                    //valueProperty.CustomAttributes.Add(new CodeAttributeDeclaration("DescriptionAttribute",
                    //    new CodeAttributeArgument(new CodeSnippetExpression("\"Gets sets, the " + result.TableName.ToLower() + " reference property for the object.\""))));
                    ...
                    valueProperty.CustomAttributes.Add(new CodeAttributeDeclaration("DataColumnForeignKeyAttribute",
                        new CodeAttributeArgument(new CodeSnippetExpression(
                            "\"" + (!String.IsNullOrEmpty(result.ForeignKeyOwner) ? result.ForeignKeyOwner + "." : "") + result.TableName +
        ...
        private void AddMethods()
        {
            ...
            CodeConditionStatement conditionalStatement = new CodeConditionStatement(
                ...
                new CodeStatement[] { 
                    new CodeExpressionStatement(
                        new CodeSnippetExpression("this.PropertyChanged(this, new PropertyChangedEventArgs(propertyName))")) });
            ...
            CodeConditionStatement changedConditionalStatement = new CodeConditionStatement(
                ...
                new CodeStatement[] { 
                    new CodeExpressionStatement(
                        new CodeSnippetExpression("this._changedPropertyNames.Add(propertyName)")) });
            ...
            CodeConditionStatement conditionalStatement1 = new CodeConditionStatement(
                ...
                new CodeStatement[] { 
                    new CodeExpressionStatement(
                        new CodeSnippetExpression("this.PropertyChanging(this, new PropertyChangingEventArgs(propertyName))")) });
        ...
        public void AddMethodsMore()
        {
            ...
            foreach (var result in _columns)
            {
                ...
                if (!result.ColumnNullable)
                {
                    ...
                    createObjectMethodData.Parameters.Add(
                        new CodeParameterDeclarationExpression(Common.LinqToDataTypes.GetLinqDataType(result.ColumnType, Common.ConnectionProvider.GetConnectionDataType(_connectionDataType)), name.ToLowerFirstLetter()));
            ...
            createObjectMethodData.Comments.Add(
                new CodeCommentStatement("<returns>The " + 
                    (!String.IsNullOrEmpty(_extendedName) ? "Data." + _extendedName + "." : "Data.") + _tableName + " entity.</returns>", true));
