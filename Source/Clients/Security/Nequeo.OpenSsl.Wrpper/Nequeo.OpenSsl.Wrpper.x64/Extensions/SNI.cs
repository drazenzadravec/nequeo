...
using Nequeo.OpenSsl.Wrpper.SSL;

namespace Nequeo.OpenSsl.Wrpper.Extensions
{
    ...
    internal class Sni
    {
        ...
        {
            return Native.SSL_ctrl(ssl, Native.SSL_CTRL_GET_SESSION_REUSED, 0, IntPtr.Zero);
        }

        private int SSL_set_tlsext_host_name(IntPtr s)
        {
            return Native.SSL_ctrl(s, Native.SSL_CTRL_SET_TLSEXT_HOSTNAME,
                                 Native.TLSEXT_NAMETYPE_host_name,
                                 _serverNamePtr);
        }
        ...
        {
            var hnptr = Native.SSL_get_servername(ssl, Native.TLSEXT_NAMETYPE_host_name);
        ...
        public int ServerSniCb(IntPtr ssl, IntPtr ad, IntPtr arg)
        {
            ...
            var extServerNamePtr = Native.SSL_get_servername(ssl, Native.TLSEXT_NAMETYPE_host_name);
