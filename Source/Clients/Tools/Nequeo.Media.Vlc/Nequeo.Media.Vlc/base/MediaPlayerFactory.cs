...
namespace Nequeo.Media.Vlc
{
    ...
    public class MediaPlayerFactory : DisposableBase, IMediaPlayerFactory, IReferenceCount, INativePointer
    {
        ...
        void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            if (exc != null)
            {
                ...
                if (error != null)
                {
                    ...
                    string errorMessage = string.Format("Method {0} supported starting libVLC version {1}", error.TargetSite.Name, ver);
                }
                else
                {
                    string msg = MiscUtils.LogNestedException(exc);
                    string errorMessage = "Unhandled exception: " + msg;
                }
            }

            if (e.IsTerminating)
            {
                string errorMessage = "Due to unhandled exception the application will terminate";
            }
        }
        ...
        private void TrySetVLCPath()
        {
            ...
            {
                string errorMessage = "Failed to set VLC path: " + ex.Message;
            }
        }
