...
namespace SevenZip
{
...
#endif
    {
        ...
        public SevenZipCompressor(string password)
            : base(password)
        {
            try
            {
        ...
        public SevenZipCompressor(bool useTemporaryPath, string temporaryPath)
        {
        ...
        private void AddFilesFromDirectoryExtension(string directory, ICollection<string> files,
            string searchPattern, string extensionRegularExpression, List<string> filesToInclude)
        {
            var di = new DirectoryInfo(directory);
            foreach (FileInfo fi in di.GetFiles(searchPattern))
            {
                if (!ScanOnlyWritable)
                {
                    ...
                    if (!String.IsNullOrEmpty(extensionRegularExpression))
                        excludeFile = Regex.IsMatch(fi.FullName.Trim(), @".*\.(" + extensionRegularExpression + @")$");

                    // Find all files that need to be included.
                    if (filesToInclude != null && !excludeFile)
                    {
                        // Should the current file be included.
                        IEnumerable<string> includeFiles = filesToInclude.Where(u => u.ToLower() == fi.FullName.Trim().ToLower());
            ...
            foreach (DirectoryInfo cdi in di.GetDirectories())
            {
                ...
                AddFilesFromDirectoryExtension(cdi.FullName, files, searchPattern, extensionRegularExpression, filesToInclude);
            }
        }
        ...
        /// <param name="filesToInclude">The list of files that are only to be compressed.</param>
        /// <remarks>Extension Regular Expression should be in the form 'jpg|JPG|gif|GIF|doc|DOC|pdf|PDF'</remarks>
        public void CompressDirectoryExtension(
            string directory, string archiveName,
            string searchPattern, bool recursion, string extensionRegularExpression, List<string> filesToInclude)
        {
            CompressDirectoryExtension(directory, archiveName, "", searchPattern, recursion, extensionRegularExpression, filesToInclude);
        }
        ...
        public void CompressDirectoryExtension(
            string directory, string archiveName,
            string password, string searchPattern, bool recursion, string extensionRegularExpression)
#else
        /// <summary>
        /// Packs all files in the specified directory.
        /// </summary>
        ...
        public void CompressDirectoryExtension(
            string directory, string archiveName,
            string password, string searchPattern, bool recursion, string extensionRegularExpression, List<string> filesToInclude)
#endif
        {
            _compressingFilesOnDisk = true;
            _archiveName = archiveName;
            using (FileStream fs = GetArchiveFileStream(archiveName))
            {
                ...
                CompressDirectoryExtension(directory, fs, password, searchPattern, recursion, extensionRegularExpression, filesToInclude);
            }
            FinalizeUpdate();
        }        
...
#endif
        {
            ...
            if (recursion)
            {
                AddFilesFromDirectoryExtension(directory, files, searchPattern, extensionRegularExpression, filesToInclude);
            }
            else
            {
                foreach (FileInfo fi in (new DirectoryInfo(directory)).GetFiles(searchPattern))
